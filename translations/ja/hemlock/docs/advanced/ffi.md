...
メモリから型付き値を直接読み取ります（`ptr_write_*` の補完）。

| 関数 | 説明 |
|----------|-------------|
| `ptr_read_i8(ptr)` | ポインタからi8を読み取り |
| `ptr_read_i16(ptr)` | ポインタからi16を読み取り |
| `ptr_read_i32(ptr)` | ポインタからi32を読み取り |
| `ptr_read_i64(ptr)` | ポインタからi64を読み取り |
| `ptr_read_u8(ptr)` | ポインタからu8を読み取り |
| `ptr_read_u16(ptr)` | ポインタからu16を読み取り |
| `ptr_read_u32(ptr)` | ポインタからu32を読み取り |
| `ptr_read_u64(ptr)` | ポインタからu64を読み取り |
| `ptr_read_f32(ptr)` | ポインタからf32を読み取り |
| `ptr_read_f64(ptr)` | ポインタからf64を読み取り |
| `ptr_read_ptr(ptr)` | ポインタからポインタを読み取り（格納された値がNULLの場合nullを返す） |
