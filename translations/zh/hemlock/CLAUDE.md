...
**类型提升：** i8 → u8 → i16 → u16 → i32 → u32 → i64 → u64 → f32 → f64（浮点总是赢，但 i64/u64 + f32 → f64 以保持精度）
...
free(p);                 // 需要手动清理
let view = b.slice(0, 16);  // 零拷贝缓冲区视图
ptr_write_f32(b, 3.14);     // ptr_read/write 直接接受缓冲区
```
...
| `atomic` | 原子操作（load、store、add、CAS、fence） |
| `bytes` | 字节序工具（bswap、hton/ntoh、端序感知 I/O） |
...
| `matrix` | 稠密矩阵操作（add、multiply、transpose、determinant、inverse） |
| `mmap` | 内存映射文件 I/O（mmap、munmap、msync） |
...
| `toml` | TOML 解析和生成 |
| `url` | URL 解析和操作 |
| `unix_socket` | Unix 域套接字（AF_UNIX 流/数据报） |
| `uuid` | UUID 生成 |
...
├── stdlib/               # 标准库 (53 个模块)
│   └── docs/             # 模块文档
├── include/              # C 头文件（hemlock_limits.h 等）
├── docs/                 # 完整文档
├── tests/                # 978+ 测试
├── examples/             # 示例程序
├── benchmark/            # 基准测试
...
- 带 22 个方法的 UTF-8 字符串
- 带 28 个方法的数组，包括 map/filter/reduce/every/some/indexOf/sort/fill
