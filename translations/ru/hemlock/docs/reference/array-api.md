...
Полный справочник по типу массивов Hemlock и всем 28 методам массивов.
...
Предварительно выделить ёмкость для будущих элементов без изменения длины массива. Полезно для избежания повторных перераспределений при массовых вставках.
...
**Параметры:**
- `n` - Минимальная ёмкость для обеспечения

**Возвращает:** `null`

**Изменяет:** Да (может изменить внутреннюю ёмкость)

**Примеры:**
```hemlock
let arr = [];
arr.reserve(1000);
print(arr.length);     // 0 - reserve не изменяет длину

// Push 1000 элементов без перераспределения
for (let i = 0; i < 1000; i++) {
    arr.push(i);
}
print(arr.length);     // 1000

// Reserve на заполненном массиве сохраняет существующие элементы
let data = [1, 2, 3];
data.reserve(1000);
print(data.length);    // 3
print(data[0]);        // 1

// Reserve меньше текущей ёмкости — no-op
data.reserve(5);
print(data.length);    // 3 (без изменений)
```
...
Найти индекс первого элемента, соответствующего функции-предикату.
...
let idx2 = arr.findIndex(fn(x) { return x > 100; }); // -1 (нет совпадений)
...
**Примечание:** В отличие от `find()`, который ищет по равенству значений, `findIndex()` использует функцию-предикат для пользовательской логики поиска.

---

#### lastIndexOf

Найти последний индекс значения, поиск с конца.
...
**Возвращает:** Индекс последнего вхождения или `-1`, если не найдено

**Примеры:**
```hemlock
let arr = ["a", "b", "c", "b", "d"];
let idx = arr.lastIndexOf("b");  // 3 (последнее вхождение)
let idx2 = arr.lastIndexOf("z"); // -1 (не найдено)
```
...
Развернуть один уровень вложенных массивов.
...
**Возвращает:** Новый массив с вложенными массивами, развёрнутыми на один уровень
...
// Не-массивные элементы сохраняются как есть
let mixed = [1, [2, 3], 4, [5]];
let flat2 = mixed.flat();
print(flat2);  // [1, 2, 3, 4, 5]

// Развёртывается только один уровень
let deep = [[1, [2, 3]], [4]];
let flat3 = deep.flat();
print(flat3);  // [1, [2, 3], 4]
```
...
Преобразовать массив в строковое представление JSON.
...
**Возвращает:** Строку JSON, представляющую массив
...
| `reserve`  | `(n: i32)`                 | `null`     | Предварительно выделить ёмкость |
...
| `findIndex`| `(predicate: fn)`          | `i32`      | Найти индекс по предикату      |
| `lastIndexOf` | `(value: any)`          | `i32`      | Найти последний индекс значения |
...
| `flat`     | `()`                       | `array`    | Развернуть один уровень вложенности |
...
| `serialize`| `()`                       | `string`   | Преобразовать в строку JSON    |
...
- Используйте `reserve(n)` для предварительного выделения ёмкости при массовых вставках
