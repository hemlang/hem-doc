...
### Присваивание с null-объединением (`??=`)

Присваивает переменной значение только если её текущее значение равно null.

| Оператор | Название                           | Пример       | Описание                                |
|----------|------------------------------------|--------------|-----------------------------------------|
| `??=`    | Присваивание с null-объединением   | `a ??= b`    | Присвоить b переменной a только если a равно null |

**Примеры:**
```hemlock
let config = null;
config ??= { timeout: 30 };    // config теперь { timeout: 30 }
config ??= { timeout: 60 };    // config не изменился (уже не null)

// Работает со свойствами и индексами
let obj = { x: null };
obj.x ??= 42;                  // obj.x теперь 42

let arr = [null, 2, 3];
arr[0] ??= "first";            // arr[0] теперь "first"
```
