...
Создать и запустить новую параллельную задачу с настройками конфигурации потока.
...
**Параметры:**
- `options` - Объект конфигурации с необязательными полями:
  - `stack_size` - Размер стека в байтах для нового потока (по умолчанию: 16 МБ)
  - `name` - Имя потока (строка, максимум 16 символов)
- `async_fn` - Асинхронная функция для выполнения
- `...args` - Аргументы для передачи функции
...
// Пользовательский размер стека (8 МБ)
...
// Пустые опции (используются значения по умолчанию — аналогично spawn())
...
**Поведение:**
- Создаёт новый поток ОС с указанной конфигурацией
- Неизвестные ключи опций молча игнорируются
- Имя потока усекается до 16 символов (ограничение pthread)
...
| `spawn_with` | `(options: object, async_fn: function, ...args)` | `task` | Запустить задачу с настройками потока (stack_size, name) |
