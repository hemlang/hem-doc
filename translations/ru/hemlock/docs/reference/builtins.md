...
Вывести значение в stdout без завершающего перевода строки.
...
**Примеры:**
```hemlock
// Построение вывода на одной строке
write("hello");
write(" ");
write("world");
print("");  // только перевод строки

// Вывод связанного списка в одну строку
let parts = [1, 2, 3];
...
**Поведение:**
- Преобразует значение в строку и выводит в stdout
- НЕ добавляет завершающий перевод строки
- Сбрасывает буфер stdout немедленно
- Используйте `print("")` после серии вызовов `write()` для добавления перевода строки
