...
Чтение типизированных значений непосредственно из памяти (дополняет `ptr_write_*`).

| Функция | Описание |
|---------|----------|
| `ptr_read_i8(ptr)` | Чтение i8 из указателя |
| `ptr_read_i16(ptr)` | Чтение i16 из указателя |
| `ptr_read_i32(ptr)` | Чтение i32 из указателя |
| `ptr_read_i64(ptr)` | Чтение i64 из указателя |
| `ptr_read_u8(ptr)` | Чтение u8 из указателя |
| `ptr_read_u16(ptr)` | Чтение u16 из указателя |
| `ptr_read_u32(ptr)` | Чтение u32 из указателя |
| `ptr_read_u64(ptr)` | Чтение u64 из указателя |
| `ptr_read_f32(ptr)` | Чтение f32 из указателя |
| `ptr_read_f64(ptr)` | Чтение f64 из указателя |
| `ptr_read_ptr(ptr)` | Чтение указателя из указателя (возвращает null, если сохранённое значение NULL) |
