...
Используйте метод `obj.delete(key)` для удаления полей:
...
Доступ к полям динамически с использованием скобочной нотации и строкового ключа:

```hemlock
let person = { name: "Alice", age: 30 };

// Чтение через скобочную нотацию
let field = "name";
print(person[field]);         // "Alice"
print(person["age"]);         // 30

// Запись через скобочную нотацию
person["city"] = "NYC";
...
Нестроковые ключи автоматически приводятся к строкам при использовании скобочной нотации. Это позволяет использовать объекты как словари с числовыми, логическими или руновыми ключами:

```hemlock
let map = {};

// Целочисленные ключи (приводятся к строке: 42 → "42")
map[0] = "zero";
map[42] = "forty-two";
print(map[0]);                // "zero"
print(map["0"]);              // "zero" (эквивалентно)

// Логические ключи (приводятся: true → "true")
map[true] = "yes";
print(map[true]);             // "yes"
print(map["true"]);           // "yes"

// Руновые ключи (приводятся: 'A' → "A")
map['A'] = "alpha";
print(map['A']);               // "alpha"
print(map["A"]);               // "alpha"

// Ключи с плавающей точкой (приводятся с полной точностью: 3.14 → "3.1400000000000001")
map[3.14] = "pi";
print(map[3.14]);              // "pi"
```

**Поддерживаемые типы приведения:**
- **Целые** (i8-i64, u8-u64): строковое десятичное представление
- **С плавающей точкой** (f32, f64): формат `%.17g` (полная точность)
- **Логические**: `"true"` или `"false"`
- **Руны**: символ в кодировке UTF-8

**Примечание:** Ключи с плавающей точкой используют полную точность IEEE 754, поэтому `3.14` становится `"3.1400000000000001"`. Если нужны точные строковые ключи для float, преобразуйте явно с помощью `"" + n`.

### Встроенные методы объектов

#### `obj.keys()`

Возвращает массив всех имён полей как строки:
...
Проверить, имеет ли объект определённое поле. Принимает строковые, целочисленные, float, bool или руновые ключи (нестроковые ключи приводятся):
...
Удалить поле из объекта. Возвращает `true`, если поле найдено и удалено, `false` в противном случае. Принимает приводимые ключи как `has()`:
