...
## Функции с телом-выражением

Для функций с одним выражением в теле используйте стрелочный синтаксис (`=>`):

```hemlock
// Именованная функция с телом-выражением
fn double(x: i32): i32 => x * 2;
fn max(a: i32, b: i32): i32 => a > b ? a : b;
fn greet(name: string): string => "Hello, " + name + "!";

// Анонимная функция с телом-выражением
let square = fn(x: i32): i32 => x * x;
let isEven = fn(n: i32): bool => n % 2 == 0;
```

**Правила:**
- Тело — одно выражение (ключевое слово `return` не нужно)
- Значение выражения автоматически возвращается
- Аннотации типов работают так же, как для функций с блочным телом
- Работает и с именованными, и с анонимными функциями

**Когда использовать:**
- Простые преобразования и предикаты
- Callback-функции, передаваемые в `map`, `filter` и др.
- Функции-обёртки, делегирующие другому вызову

```hemlock
// Отлично для операций с массивами
let nums = [1, 2, 3, 4, 5];
...
Модификатор `ref` передаёт ссылку на переменную вызывающего кода, позволяя функции модифицировать её напрямую:

### Базовые ref-параметры
...
- Без `ref` примитивы передаются по значению (копируются)
